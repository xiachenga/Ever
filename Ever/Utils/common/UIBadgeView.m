...
+ (CGSize)badgeSizeWithStr:(NSString *)badgeValue font:(UIFont *)font{
 ...
 //   CGSize badgeSize = [badgeValue getSizeWithFont:font constrainedToSize:CGSizeMake(100, 20)];
    
    CGSize badgeSize=[badgeValue boundingRectWithSize:CGSizeMake(100, 20) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]} context:nil].size;
...
- (CGRect)badgeFrameWithStr:(NSString *)badgeValue{
    ...
    CGRect badgeFrame = CGRectMake(25, 0, badgeSize.width +8, badgeSize.height +8);//8=2*2（红圈-文字）+2*2（白圈-红圈）
    return badgeFrame;
}
