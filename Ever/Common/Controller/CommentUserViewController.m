...
@interface CommentUserViewController ()<ThreeUserCellDelegate>
...
- (void)viewDidLoad {
    ...
    self.title=LOCALIZATION(@"CommentRanking");
...
- (void)reloadData
{
    ...
    NSString *string=[kSeverPrefix stringByAppendingString:@"user/ranking/2/60/1/0"];
    [HttpTool get:string params:nil success:^(id responseobj) {
        
        CLog(@"%@",responseobj);
        
        NSArray *array=[RankResult objectArrayWithKeyValuesArray:responseobj];
        [self.commentUserArray addObjectsFromArray:array];
        
        
        NSRange range=NSMakeRange(0, 3);
        NSIndexSet *indexset=[NSIndexSet indexSetWithIndexesInRange:range];
        NSArray *threeUserArray=[self.commentUserArray objectsAtIndexes:indexset];
        self.threeUserArray=threeUserArray;
        
        [self.commentUserArray removeObjectsAtIndexes:indexset];
...
//- (void)loadMoreData
//{
//    NSString *string=[NSString stringWithFormat:@"user/ranking/2/60/%d/%ld",self.num,self.lastID];
//    
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return self.commentUserArray.count+1;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
        ...
        default:
        {
            RankResult *rangResult=self.commentUserArray[indexPath.row-1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        ...
        {
            RankResult *rankResult=self.commentUserArray[indexPath.row-1];
