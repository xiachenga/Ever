...
- (void)layoutSubviews {
    ...
    if (hasImage) {
     ...
     //   self.imageView.frame = CGRectInset(CGRectMake(0, y, CGRectGetWidth(frame), height), inset, inset);
        self.imageView.frame =CGRectMake(0, y, 150, 150);
    ...
    if (hasText) {
        ...
        self.titleLabel.frame = CGRectMake(left, y+10, CGRectGetWidth(frame), 130);
    }
    else {
...
- (void)layoutAsList {
   ...
   // self.menuView.frame=CGRectMake(0, 0, 320, 480);
...
//    CGFloat headerOffset = self.headerView.bounds.size.height;
   ...
   // self.menuView.frame = [self menuFrameWithWidth:width height:height center:self.menuCenter headerOffset:headerOffset];
    self.menuView.frame=CGRectMake(0, 0, kScreen_Width, kScreen_Height);
   ...
   //     NSInteger offset = 0;
        ...
        [subItems enumerateObjectsUsingBlock:^(RNMenuItemView *itemView, NSUInteger idx, BOOL *stop) {
            ...
            itemView.frame = CGRectMake((kScreen_Width-150)*0.5, i * (150+(kScreen_Height-64-330)/3)+((kScreen_Height-64-330)/3+64),150, 150);
            
        //   itemView.backgroundColor=[UIColor redColor];
            
           
           // itemView.frame=CGRectMake(150, 100+idx*100, 100, 100);
        }];
    }
}
  ...
  //  [self.view addSubview:self.blurView];
    ...
    [self createScreenshotAndLayoutWithScreenshotCompletion:^{
        if (animated) {
            ...
            opacityAnimation.duration = self.animationDuration * 1.0f;
